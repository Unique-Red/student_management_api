...
from ..models import Student, Course, CourseRegistered, Grade
...
class StudentListAPI(Resource):
    ...
    def post(self):
        student = Student(matric_number=student_namespace.payload['matric_number'], first_name=student_namespace.payload['first_name'], last_name=student_namespace.payload['last_name'], email=student_namespace.payload['email'], password=generate_password_hash(student_namespace.payload['password']))
        if Student.query.filter_by(matric_number=student_namespace.payload['matric_number']).first():
...
class StudentAPI(Resource):
    ...
    def put(self, id):
        student = Student.query.get_or_404(id)
        student.matric_number = student_namespace.payload['matric_number']
